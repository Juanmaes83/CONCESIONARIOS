...
try{
 ...
 await page.waitForFunction(()=>document.documentElement.dataset.dealerTrackReady==='true',null,{timeout:15000});await page.waitForTimeout(1200);
 const signature=page.locator('#signature');await signature.scrollIntoViewIfNeeded();await page.waitForTimeout(500);
 const signatureBox=await signature.boundingBox();check('showcase is in viewport',!!signatureBox&&signatureBox.y<1000&&signatureBox.y+signatureBox.height>0,JSON.stringify(signatureBox));
 ...
 const shellLoc=page.locator('.orbit-shell');const shell=await shellLoc.boundingBox(),left=await box(5),hero=await box(0),right=await box(1);const lf=frac(left,shell),rf=frac(right,shell);
 check('orbit shell is in viewport',!!shell&&shell.y<1000&&shell.y+shell.height>0,JSON.stringify(shell));
 ...
 await signature.screenshot({path:'qa/nova-v4-rest.png'});
 ...
 await signature.screenshot({path:'qa/nova-v4-model-x.png'});
